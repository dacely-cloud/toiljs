...
**2. WASM isolation and density.** Each site compiles to its own tiny, [sandboxed](./how-it-works.md#what-build-produces) WASM module that starts fast and cannot touch another tenant's files, memory, or network. Hard per-request limits (a memory cap in the tens of MiB, a **gas meter** that cuts off a looping handler, rate limits, and hostile-wasm containment) let one box safely hold many tenants at once, which is what makes compute in many cities affordable instead of a luxury.

**3. Allocation-free hot path.** The code that runs on every request wastes nothing: zero-copy request and response envelopes, reused byte buffers instead of per-request allocations, and no garbage-collection pauses (so no random latency spikes under load). This earns latency with lean code rather than by overprovisioning hardware to hide slow code, which is a bar toil holds itself to explicitly ([the RSG rubric](./design-principles.md)).
...
**5. Modern transport.** The edge speaks HTTP/3 over QUIC (with graceful fallback to HTTP/2 and HTTP/1.1, plus WebTransport for realtime) and uses userspace networking (DPDK, multi-queue) to move packets across many CPU queues in parallel instead of paying the general-purpose OS network path per packet. That keeps the connection-level cost of each request low as traffic grows. You configure none of it.
